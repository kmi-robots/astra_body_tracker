...
public:
    explicit astra_body_tracker_node(std::string name) : _name(name) {
        ROS_INFO_STREAM(name << ": Initializing");
        last_id_ = -1;
    ...
    ~astra_body_tracker_node() {
        ROS_INFO_STREAM("astra_body_tracker_node shutting down");
    }
...
private:
    ...
    void output_floor(astra_bodyframe_t bodyFrame) {
        ...
        if (rc != ASTRA_STATUS_SUCCESS) {
            std::cout << "Error " << rc << " in astra_bodyframe_floor_info()" << std::endl;
            return;
        }
        ...
        if (floorDetected != ASTRA_FALSE) {
            std::cout << "Floor plane: [" << floorPlane->a << ", " << floorPlane->b << ", " << floorPlane->c << ", "
                      << floorPlane->d << "]" << std::endl;

            const int32_t bottomCenterIndex = floorMask->width / 2 + floorMask->width * (floorMask->height - 1);
            std::cout << "Floor mask: width: " << floorMask->width << " height: " << floorMask->height
                      << " bottom center value: " << floorMask->data[bottomCenterIndex] << std::endl;
        }
    }
    ...
    void output_body_mask(astra_bodyframe_t bodyFrame) {
        ...
        if (rc != ASTRA_STATUS_SUCCESS) {
            std::cout << "Error " << rc << " in astra_bodyframe_bodymask()" << std::endl;
            return;
        }
    }
    ...
    void output_bodyframe_info(astra_bodyframe_t bodyFrame) {
        ...
        if (rc != ASTRA_STATUS_SUCCESS) {
            std::cout << "Error " << rc << " in astra_bodyframe_info()" << std::endl;
            return;
        }
    }
    ...
    void output_bodies(astra_bodyframe_t bodyFrame) {
        ...
        for (i = 0; i < bodyList.count; ++i) {
            ...
            body_tracker_msgs::Skeleton skeleton_data;
